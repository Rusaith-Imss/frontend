...
import {
  ClipboardList, Trash2, LogOut, Maximize, Minimize, Calculator, LayoutDashboard, PauseCircle, RefreshCw, Printer,
} from "lucide-react";
...
const POSForm = () => {
  ...
  useEffect(() => {
    setProducts((prevProducts) =>
      prevProducts.map((product) => {
        ...
        const discountPerUnit = Math.max(parseFloat(product.mrp || 0) - newPrice, 0);

        return {
          ...product,
          price: newPrice,
          discount: discountPerUnit,
          total: newPrice * (product.qty || 1) - (discountPerUnit * (product.qty || 1)),
        };
      })
    );
  }, [saleType]);
  ...
  const addProductToTable = () => {
    ...
    const price = saleType === "Wholesale" ? selectedProduct.wholesale_price : selectedProduct.sales_price;
    ...
    if (newQty === "" || /^[0-9]*\.?[0-9]*$/.test(newQty)) {
      setProducts((prevProducts) =>
        prevProducts.map((product, i) =>
          i === index ? { ...product, qty: parseFloat(newQty) || 0, total: parseFloat(newQty) * product.price } : product
        )
      );
    }
  };
      ...
      prevProducts.map((product, i) =>
        i === index
          ? {
            ...product,
            price: newPrice,
            total: newPrice * (product.qty || 1) - (product.discount * (product.qty || 1)),
          }
          : product
      )
    );
  ...
  const calculateTotals = () => {
    const totalQty = products.reduce((acc, product) => acc + (product.qty || 0), 0);
    const subTotal = products.reduce((acc, product) => acc + ((product.mrp || 0) * (product.qty || 0)), 0);
    const totalItemDiscounts = products.reduce((acc, product) => acc + ((product.discount || 0) * (product.qty || 0)), 0);
        ...
        <div className="flex items-center justify-between w-full bg-slate-500 mt-0 mb-1 p-4 rounded-lg shadow-md">
          ...
          <div className="flex items-center space-x-4">
            ...
            <button
              ...
              title="Calculator"
              onClick={() => { setShowCalculatorModal(true); setIsOpen(true); }}
        ...
        <div className="grid grid-cols-3 gap-4">
              ...
              {searchQuery.trim() !== "" && searchResults.length > 0 && (
                <ul className="absolute z-50 w-60 border rounded-lg top-full bg-slate-50/5 backdrop-blur-sm shadow-lg max-h-60 overflow-auto">
                  {searchResults.map((item, index) => (
                    <li
                      key={index}
                      className={`p-2 cursor-pointer hover:bg-slate-800 ${index === selectedSearchIndex ? "bg-slate-500 text-amber-400" : ""}`}
            ...
            <table className="w-full border border-gray-300">
              <thead className="bg-gray-700 border text-amber-600 border-gray-300">
                <tr className="border border-gray-300">
                  ...
                  <th className="border border-gray-500 px-2 py-1">U.Discount</th>
              ...
              <tbody className="text-center">
                {products.map((product, index) => (
                  <tr key={index} className="border border-gray-300">
                    ...
                    <td className="border border-gray-500 text-left px-2 py-1">{product.product_name}</td>
                    <td className="border border-gray-500 text-right px-2 py-1">{formatNumberWithCommas(product.mrp)}</td>
                    ...
                    <td className="border border-gray-500 px-2 py-1">
                      <input
                        ...
                        value={product.price}
                        readOnly
                        onChange={(e) => updateProductPrice(index, parseFloat(e.target.value) || 1)}
                      />
                    </td>
                    ...
                    <td className="border border-gray-500 text-right px-2 py-1">
                      {formatNumberWithCommas((product.qty * product.price).toFixed(2))}
                    </td>
          ...
          <div className="p-4 min-w-80 w-full rounded-lg shadow-lg backdrop-blur-md bg-slate-700 bg-opacity-40">
            ...
            <div className="p-4 mt-4 text-center bg-transparent rounded-lg shadow-lg">
              <p>Total Qty: {formatNumberWithCommas((calculateTotals().totalQty || 0).toFixed(1))}</p>
              <p>Item Discounts: {formatNumberWithCommas((calculateTotals().totalItemDiscounts || 0).toFixed(2))} Rs.</p>
              <p>Additional Discount: {formatNumberWithCommas((calculateTotals().additionalDiscount || 0).toFixed(2))} Rs.</p>
              <p>Total Discount: {formatNumberWithCommas((calculateTotals().totalDiscount || 0).toFixed(2))} Rs.</p>
              <p>Sub Total: {formatNumberWithCommas((calculateTotals().subTotal || 0).toFixed(2))} Rs.</p>
              <h2 className="text-2xl font-bold text-green-500">
                Total: {formatNumberWithCommas((calculateTotals().total || 0).toFixed(2))} Rs.
              </h2>
            </div>
