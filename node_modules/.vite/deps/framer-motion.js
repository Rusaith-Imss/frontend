...
import {
  u
} from "./chunk-HMMN2ZRY.js";
import {
  J,
  g,
  k,
  x
} from "./chunk-UJFOHT4D.js";
import {
  __commonJS,
  __export
...
var PresenceChild = ({ children, initial, isPresent: isPresent2, onExitComplete, custom, presenceAffectsLayout, mode, anchorX }) => {
  ...
  if (mode === "popLayout") {
    children = u(PopChild, { isPresent: isPresent2, anchorX, children });
  }
...
var AnimatePresence = ({ children, custom, initial = true, onExitComplete, presenceAffectsLayout = true, mode = "sync", propagate = false, anchorX = "left" }) => {
  ...
  const [diffedChildren, setDiffedChildren] = h(presentChildren);
  const [renderedChildren, setRenderedChildren] = h(presentChildren);
  ...
  return u(k, { children: renderedChildren.map((child) => {
    ...
    return u(PresenceChild, { isPresent: isPresent2, initial: !isInitialRender.current || initial ? void 0 : false, custom, presenceAffectsLayout, mode, onExitComplete: isPresent2 ? void 0 : onExit, anchorX, children: child }, key);
  }) });
};
...
init_compat_module();
var DeprecatedLayoutGroupContext = J(null);
...
function useForceUpdate() {
  ...
  const [forcedRenderCount, setForcedRenderCount] = h(0);
  const forceRender = q(() => {
...
init_compat_module();
var LazyContext = J({ strict: false });
...
function LazyMotion({ children, features, strict = false }) {
  const [, setIsLoaded] = h(!isLazyBundle(features));
...
init_compat_module();
var SwitchLayoutGroupContext = J({});
...
function useSpring(source, config = {}) {
  ...
  const latestValue = A(initialValue);
  const latestSetter = A(noop);
...
function useReducedMotion() {
  ...
  const [shouldReduceMotion] = h(prefersReducedMotion.current);
  if (true) {
...
function useCycle(...items) {
  const index = A(0);
  const [item, setItem] = h(items[index.current]);
  const runCycle = q(
    (next) => {
...
function useInView(ref, { root, margin, amount, once = false, initial = false } = {}) {
  const [isInView, setInView] = h(initial);
  y(() => {
...
function useAnimatedState(initialState) {
  const [animationState, setAnimationState] = h(initialState);
